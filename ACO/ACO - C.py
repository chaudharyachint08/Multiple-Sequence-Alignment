...
file = open(input('Enter File Containing DNA Sequences to be aligned using ACO-C\n'))
...
class Ant:
    ...
    def make_drift(self,other):
        ...
        k,val = -1,-1
        for i in all_pos:
            new_val = lcs2( self.seq[self.pos:self.pos+self.length] , other.seq[i:i+self.length])
            if new_val > val:
                k = i
                val = new_val
